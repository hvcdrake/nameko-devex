...
class TestDeleteProduct(object):
    ...
    def test_delete_product_not_found(self, gateway_service, web_session):
        ...
        # call the gateway service to get product 777
        response = web_session.delete('/products/777')
        assert response.status_code == 404
        payload = response.json()
        assert payload['error'] == 'PRODUCT_NOT_FOUND'
        assert payload['message'] == 'missing'
...
class TestGetOrder(object):
    ...
    def test_can_get_order(self, gateway_service, web_session):
        ...
        gateway_service.products_rpc.list_by_keys.return_value = [
            {
                'id': 'the_odyssey',
                'title': 'The Odyssey',
                'maximum_speed': 3,
        ...
        assert [call(*["the_odyssey","the_enigma"])] == gateway_service.products_rpc.list_by_keys.call_args_list
...
class TestCreateOrder(object):
    ...
    def test_can_create_order(self, gateway_service, web_session):
        ...
        assert gateway_service.products_rpc.list_by_keys.call_args_list == [call(*['the_odyssey'])]
